...
private ["_jet", "_action", "_distanceA", "_distanceB", "_msg", "_jet_Alt", "_angle",
	"_jdgNimArrestor_Alt", "_jdgNimArrestor"];
...
_cutlight = 0;
while {(_jet distance _jdgNimArrestor) > 25} do 
{
	_distanceA = _jet distance _jdgNimArrestor;
	...
	switch (true) do
	{
		case ((_angle > 0.034) && (_angle < 0.046) && ((_jet distance _jdgNimArrestor) < 150)): {TitleRsc ["IFLOLS_cut", "PLAIN", 0]; _cutLight = 1;}; //cut lights on, good landing, exit script
...
_jet addAction[_msg, TTT_fnc_ifolsCalc, [], 2, false, true, "", 
