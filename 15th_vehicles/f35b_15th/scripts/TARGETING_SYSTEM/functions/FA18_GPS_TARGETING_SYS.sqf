...
If (_FA18 animationphase "gps_targeting_switch" > 0.01) exitWith {if ((player == _FA18_PILOT) or (player == _FA18_WSO)) then {titleText ["\n \n GPS/INS target is SET/ACTIVE, \n clear the previuos taget first...", "PLAIN DOWN",1];};};
sleep 0.01;
_FA18 animate ["gps_targeting_switch",1];
...
waituntil {mapclick};
target_pos = clickpos;
target = "JS_JC_FA18_GPSTarget" createVehicleLocal target_pos;
...
while {(_FA18 animationphase "gps_targeting_switch" < 0.01) and (alive _FA18)} do 
{
